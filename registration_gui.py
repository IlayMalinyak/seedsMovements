...
def plot_values(registration_method):
    global metric_values, multires_iterations, ax, fig
    res_event_on_legend = False
    try:
        metric_values.append(registration_method.GetMetricValue())
    except AttributeError:
        metric_values.append(registration_method.GetMetric())
    # Plot the similarity metric values
    ax.plot(metric_values, 'r')
    if not res_event_on_legend:
        ax.plot(multires_iterations, [metric_values[index] for index in multires_iterations], 'b*',
                label="multi resolution event")
        res_event_on_legend = True
